...
function bindCommon(){
  ...
  const publish=()=>{const fields={};document.querySelectorAll('[data-field]').forEach(field=>{if(field.type==='radio'&&!field.checked)return;const value=field.type==='checkbox'?(field.checked?'완료':''):field.value;save(field);if(value)fields[field.dataset.label||'학생 응답']=value});send({type:'activity',deck:state.deck,slide:state.slide,fields})};
  document.querySelectorAll('[data-field]').forEach(input=>{input.oninput=()=>save(input);if(input.type==='radio'||input.type==='checkbox')input.onchange=publish;else input.onkeydown=event=>{if(event.key==='Enter'&&!event.isComposing){event.preventDefault();publish()}}});
