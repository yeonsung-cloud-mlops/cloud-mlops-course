...
function studentDock(slide){const key=`field-${state.deck}-${state.slide}-question`;return `<div class="student-response"><label for="studentResponse">질문</label><input id="studentResponse" data-field data-label="학생 질문" data-store="${attr(key)}" value="${attr(localStorage.getItem(key)||'')}" placeholder="궁금한 점을 입력하세요"></div>${attachmentLinks()}${resourceLinks(slide)}`}
