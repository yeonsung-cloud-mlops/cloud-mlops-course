...
const role = params.get('role') === 'teacher' ? 'teacher' : roomId ? 'student' : 'landing';
const teacherKey = params.get('key') || '';
let socket, connected=false, state={slide:0,revealed:false,timerEnd:null},presence={students:0,completed:0},completed=false,retry=0,timerHandle;
...
function connect(){
  ...
  socket.onmessage=event=>{const message=JSON.parse(event.data);if(message.type==='state')state=message.state;if(message.type==='presence')presence=message;if(message.type==='expired'){app.innerHTML='<section class="error"><h1>수업이 종료되었습니다</h1><p>강사에게 새 수업 코드를 받아주세요.</p></section>';return}render()};
}
...
function slideContent(slide,index){
  ...
  if(slide.fields)body+=`<div class="fields">${slide.fields.map((label,i)=>`<div class="field"><label>${safe(label)}</label><input data-field="${index}-${i}" value="${safe(localStorage.getItem(`field-${index}-${i}`)||'')}"></div>`).join('')}</div>`;
...
function bindCommon(){
  document.querySelectorAll('[data-field]').forEach(input=>input.oninput=()=>localStorage.setItem(`field-${input.dataset.field}`,input.value));
