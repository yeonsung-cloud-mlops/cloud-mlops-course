...
function diagnosticSummary(meta,{publicView=false}={}){const responses=currentActivityResponses().map(responseData),total=Math.max(roster.length,presence.students,responses.length),questions=meta.choices||[];const rows=questions.map(question=>{const values=responses.map(item=>item.fields[question.label]).filter(Boolean),correct=values.filter(value=>value===question.correct).length,unknown=values.filter(value=>value==='모르겠다').length,wrong=values.length-correct-unknown,missing=Math.max(0,total-values.length),rate=total?Math.round(correct/total*100):0;return `<article><div><strong>${safe(question.label)}</strong><span>정답 ${correct} · 오답 ${wrong} · 모름 ${unknown} · 미응답 ${missing}</span></div><b>${rate}%</b></article>`}).join('');return `<section class="diagnostic-summary ${publicView?'public':''}"><header><strong>기초 진단 실시간 집계</strong><span>${questions.length}문항 · ${responses.length}/${total||0}명 응답 중</span></header>${rows||'<p>학생이 답하면 문항별 분포가 표시됩니다.</p>'}</section>`}
