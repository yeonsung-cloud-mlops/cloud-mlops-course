...
function choiceSummary(meta,{publicView=false}={}){const responses=currentActivityResponses().map(responseData),total=Math.max(presence.students,responses.length);return `<section class="choice-live-summary ${publicView?'public':''}"><header><strong>객관식 실시간 응답</strong><span>${responses.length}/${total||0}명 응답 중</span></header>${meta.choices.map(question=>{const values=responses.map(item=>item.fields[question.label]).filter(Boolean),answered=values.length;return `<article><h3>${safe(question.label)} <small>${answered}명</small></h3>${question.options.map(option=>{const count=values.filter(value=>value===option).length,percent=answered?Math.round(count/answered*100):0;return `<div><span>${safe(option)}</span><i><b style="width:${percent}%"></b></i><em>${count}명 · ${percent}%</em></div>`}).join('')}</article>`}).join('')}</section>`}
function teacherResponsePreview(){const meta=currentInteraction();if(meta?.diagnostic)return diagnosticSummary(meta);const choiceLabels=new Set((meta?.choices||[]).map(question=>question.label)),choice=meta?.choices?.length?choiceSummary(meta):'';const rows=currentActivityResponses().flatMap((response,index)=>{const item=responseData(response,index);return Object.entries(item.fields).filter(([label,value])=>value&&!choiceLabels.has(label)).map(([label,value])=>`<div class="teacher-response"><strong>${safe(item.name)} · ${safe(label)}</strong><span>${safe(value)}</span></div>`)}).slice(0,8);const written=rows.length||!choice?`<section class="teacher-response-list"><div><strong>현재 장표 답안</strong><span>${rows.length?`${rows.length}개 표시`:'대기 중'}</span></div>${rows.length?rows.join(''):'<p>학생이 입력하면 이름과 답안이 여기에 표시됩니다.</p>'}</section>`:'';return choice+written}
...
function activityBoard(){const meta=currentInteraction();if(meta?.diagnostic)return `<aside class="activity-board diagnostic-board">${diagnosticSummary(meta,{publicView:true})}</aside>`;const choiceLabels=new Set((meta?.choices||[]).map(question=>question.label)),choice=meta?.choices?.length?choiceSummary(meta,{publicView:true}):'';const rows=currentActivityResponses().flatMap((response,index)=>{const item=responseData(response,index);return Object.entries(item.fields).filter(([label,value])=>value&&!choiceLabels.has(label)).map(([label,value])=>`<div class="response-card"><strong>학생 ${index+1} · ${safe(label)}</strong><span>${safe(value)}</span></div>`)});return `<aside class="activity-board${choice?' choice-board':''}">${choice||'<h2>학생 실시간 답안</h2>'}${rows.length?rows.join(''):choice?'':'<p class="response-empty">아직 입력된 답안이 없습니다.</p>'}</aside>`}

function bindCommon(){
  const save=input=>{const value=input.type==='checkbox'?(input.checked?'완료':''):input.value;if(input.dataset.store)localStorage.setItem(input.dataset.store,value)};
  const payload=()=>{const fields={};document.querySelectorAll('[data-field]').forEach(field=>{if(field.type==='radio'&&!field.checked)return;const value=field.type==='checkbox'?(field.checked?'완료':''):field.value;save(field);if(value)fields[field.dataset.label||'학생 응답']=value});return {type:'activity',deck:state.deck,slide:state.slide,fields}};
  const publish=()=>send(payload());
  const publishSoon=()=>{const pending=payload();clearTimeout(activityTimer);activityTimer=setTimeout(()=>send(pending),250)};
  document.querySelectorAll('[data-field]').forEach(input=>{input.oninput=()=>save(input);if(input.type==='radio'||input.type==='checkbox')input.onchange=publishSoon;else input.onkeydown=event=>{if(event.key==='Enter'&&!event.isComposing){event.preventDefault();clearTimeout(activityTimer);publish()}}});
...
function bindTeacher(){
  const teamManagement=document.querySelector('.team-management');if(teamManagement)teamManagement.ontoggle=()=>{teamManagementOpen=teamManagement.open};
