...
- Q: Stop과 Terminate 중 매주 뭘 하나요? → A: 매주는 **Stop**입니다. Terminate 하면 그 안에 설치한 것들이 전부 날아가서 다음 주에 처음부터 다시 해야 합니다. Terminate는 14주차에 합니다.
...
cat ~/work/setup.sh
```
> 오늘 20분 걸린 작업이 이 파일 하나면 3분입니다. **이게 자동화의 첫걸음**입니다. 11주차에 이 개념이 크게 확장됩니다.

### 팀 프로젝트 연결

- 오늘 만든 인스턴스 `mlops-<학번>` 은 **3~12주차 내내 여러분의 개발 서버**다. 매주 Stop/Start를 반복하며 쓴다.
...
- ⚠ **서버 안의 파일은 언제든 날아갈 수 있다.** 중요한 코드는 S3나 GitHub(11주차)에 올려두는 습관을 지금부터 들인다.
...
> ⚠ 중지해도 **EBS 30GB 요금(월 약 $3)은 계속 나간다.** 이건 학기 운영상 감수하는 비용이며, 14주차에 Terminate로 완전히 정리한다.
...
| Terminate (종료) | 폐기 | 인스턴스와 EBS까지 삭제. **복구 불가.** 14주차에만 |
