...
> "그리고 그 상자를 AWS에 올려 두면, 다른 서버에서 한 줄로 내려받아 똑같이 돌릴 수 있습니다. 오늘 끝나면 여러분 이름의 리포지토리에 여러분이 만든 상자가 올라가 있을 겁니다. 그 상자가 **7주차에 API가 되고, 8주차에 배포되고, 11주차에 자동으로 만들어집니다.** 오늘이 그 시작입니다."
...
- 오늘 만든 ECR 리포지토리는 **11주차 GitHub Actions가 자동으로 push할 목적지**입니다. `repositoryUri`를 팀 README에 적어 두세요.
...
> ⚠ **비용 메모**: ECR 저장 요금은 GB당 월 약 $0.10입니다. 오늘 올린 이미지가 500MB면 월 $0.05로 무시할 수준입니다. 다만 **매주 태그를 추가하면서 정리를 안 하면 학기말에 수 GB가 됩니다.** 그리고 EC2 디스크에 쌓이는 빌드 캐시는 EBS 요금에 직결되므로, 정리 타임에 `docker system df` 를 보는 습관을 들이세요. 리전이 다르면 pull에 데이터 전송 요금이 붙으니 **ECR도 반드시 `ap-northeast-2`** 에 만듭니다.
