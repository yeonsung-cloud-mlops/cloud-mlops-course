...
- Q: `/health`는 왜 필요하죠? 예측만 되면 되는 것 아닌가요? → A: 나중에 서버를 자동으로 관리하는 도구들이 "이 서버 살아있나?"를 1분마다 물어봅니다. 그때 예측을 시키면 낭비니까, 가볍게 "네 살아있어요"만 답하는 문을 따로 두는 겁니다. 12주차 모니터링에서 이 문을 씁니다.
...
4. **(선택)** 응답에 `latency_ms`(예측에 걸린 밀리초)를 추가한다. 12주차 모니터링에서 이 값을 쓴다.
