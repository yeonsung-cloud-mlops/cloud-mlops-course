...
>
> 마지막으로 시연 팁 하나. 오늘 5분 시연에서 가장 흔한 실패는 "현장에서 처음 눌러보는 것"입니다. 반드시 **팀에서 한 명이 리허설로 끝까지 눌러본 뒤** 발표에 들어가세요. 그리고 13주차에도 말하겠지만, **화면 녹화 백업**은 지금부터 습관을 들이세요.
...
오늘 실습 B의 산출물이 곧 **M2 중간 시연(15점)의 제출 실체**다. 그리고 여기서 만든 `docker-compose.yml`은 11주차 GitHub Actions 자동 배포에서 **배포 스크립트가 실행할 대상**이 된다. 즉 오늘 파일을 팀 저장소에 커밋해 두면 11주차에 그대로 재사용된다.
...
2. **(필수)** M2 시연에서 받은 지적사항을 팀 이슈로 3개 이상 등록하고 담당자를 지정한다. 13주차 코칭 때 이 목록을 점검한다.
3. **(권장)** UI에 "최근 예측 10건" 표를 추가한다. `st.session_state`에 리스트로 쌓아 `st.dataframe()`으로 표시하면 된다. 12주차 모니터링의 예고편이다.
4. **(선택)** `docker compose logs api > api.log` 로 로그를 파일로 남겨 보고, 어떤 정보가 부족한지 한 문단으로 적어 온다. 12주차 로그 설계에 쓴다.
