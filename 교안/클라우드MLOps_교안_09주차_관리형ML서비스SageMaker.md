# 09주차 — 관리형 ML 서비스 (Amazon SageMaker AI) ⚠ 비용 주의 회차
...
오늘 만드는 것 중 3가지는 "켜두면 계속 돈이 나간다"
  1) Studio 노트북 스페이스   (ml.t3.medium  약 $0.06/시간)
  2) 학습 잡                   (실행되는 동안만 과금 — 끝나면 자동 종료)
  3) 실시간 엔드포인트         (ml.m5.large  약 $0.14/시간 = 하루 $3.4)
...
>
> **비용.** 이게 재밌습니다. 우리 EC2 `t3.medium`은 시간당 약 $0.052입니다. SageMaker 엔드포인트 `ml.m5.large`는 약 $0.14입니다. 대략 **2.7배**입니다. 왜 비쌀까요? 관리 서비스 값이 포함돼 있기 때문입니다. 그런데 여기서 착시가 있습니다. 우리 EC2는 API 말고 다른 일도 시킬 수 있죠. 엔드포인트는 그 모델 하나만 서빙합니다. 모델이 5개면 엔드포인트도 5개고, 요금도 5배입니다. **모델 수가 늘수록 격차가 벌어집니다.**
>
...
| 비용(동급 사양) | **낮다** (`t3.medium` 약 $0.052/h) | 높다 (`ml.m5.large` 약 $0.14/h, 약 2.7배) |
...
> 네 가지를 구분하세요.
>
> 첫째, **Studio 스페이스(Space)**. 여러분이 노트북을 쓰기 위해 켜는 작은 서버입니다. `ml.t3.medium`이면 시간당 약 $0.06. **브라우저를 닫아도 안 꺼집니다.** 반드시 스페이스를 Stop 해야 합니다.
>
...
|---|---|---|---|
| Studio 스페이스 | 시간당 (`ml.t3.medium` 약 $0.06/h) | ✕ (유휴 설정 시 △) | 수업 끝나면 **Stop** |
| 학습 잡 | 실행 시간만 (초 단위) | ✅ 자동 | 조치 불필요 |
| **실시간 엔드포인트** | **시간당 (`ml.m5.large` 약 $0.14/h)** | **✕** | **호출 직후 즉시 삭제** |
...
| **비용** | 서버 단가 (시간당) | `t3.medium` 약 $0.052 | `ml.m5.large` 약 $0.14 | |
...
이 비교표는 **최종 발표(14주차)의 '기술 선택 근거' 슬라이드가 된다.** 최종 산출물 루브릭의 "문서화·발표 — 문제정의·의사결정 근거가 명확" 항목에서 직접 평가된다. "왜 SageMaker를 안 썼나요?"라는 질문은 심사에서 반드시 나오며, 오늘의 표가 그 답이다. 팀 저장소 `docs/tech-choice.md`로 커밋한다.
...
**일시**: 2026년 ___월 ___일 9주차 · **점검 시각**: 03:45–03:50 · **조교**: ____________
