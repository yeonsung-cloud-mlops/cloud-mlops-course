...
| 사전 준비 | ① ⚠ **개강 2주 전까지 Bedrock 모델 액세스 신청·승인 완료 확인 (서울 리전 `ap-northeast-2`)** — 승인에 시간이 걸리므로 절대 미루지 않는다 ② **서울 리전에서 실제 호출 가능한 모델 ID 목록을 개강 직전 갱신**해 배포 (모델 라인업과 교차 리전 추론 프로파일 접두어는 수시로 바뀐다) ③ 학생 IAM 정책에 `bedrock:InvokeModel`, `bedrock:Converse`, `bedrock:ListFoundationModels` 허용 ④ 강사가 수업 당일 아침 `list-foundation-models`로 최종 확인 ⑤ 콘솔 UI 캡처 갱신 ⑥ `week-10-done` 태그 준비 |
...
>
> 우리 수업 예상 비용은 학생 1인당 **약 $1** 입니다. 지난주 엔드포인트 하루 방치 비용($3.4)보다 적습니다. 다만 방심하지 마세요. **반복문 안에서 LLM을 부르는 코드**를 실수로 돌리면 순식간에 수천 번 호출됩니다. 오늘 실습에서 `for` 문 안에 Bedrock 호출을 넣을 때는 반드시 건수를 제한하세요.
...
3. 좌측 하단 **Model access**(모델 액세스) 클릭
4. 목록에서 오늘 사용할 모델의 **Access status**가 `Access granted`인지 확인

- 확인 포인트: `Access granted` 표시. `Available to request`나 `In progress`면 즉시 강사에게 알린다.
...
```bash
git clone --branch week-10-done https://github.com/<강사계정>/mlops-2026-reference.git ~/week10-done
```
...
오늘 만든 엔드포인트는 최종 산출물 루브릭의 **"파이프라인 완성도"**에 더해 **차별화 요소**로 평가된다. 특히 C 도메인(텍스트 리뷰) 팀은 이 기능이 프로젝트의 핵심 기능 중 하나가 된다. 11주차 GitHub Actions 자동 배포에서는 오늘 만든 `v2` 이미지가 배포 대상이 되고, 12주차 모니터링에서는 **토큰 사용량이 커스텀 메트릭 후보**가 된다(`input_tokens`, `output_tokens`를 CloudWatch로 보낸다).
...
4. **(선택)** 호출 1회당 예상 비용을 계산해 본다. `input_tokens`, `output_tokens`와 배포된 모델 단가표를 곱해, "우리 서비스가 하루 500번 호출되면 월 얼마인가"를 산출한다. 9주차 비교표에 한 행으로 추가한다.
