...
- Q: 모델을 바꿔도 이 파이프라인이 도나요? → A: 모델 파일을 이미지에 넣었다면 그대로 돕니다. 모델을 S3에서 받아오는 구조라면 코드가 안 바뀌었으니 파이프라인이 안 돕니다. 그 경우엔 12주차에 배울 재학습 주기와 연결해서 수동 트리거(`workflow_dispatch`)로 돌립니다. 오늘 템플릿에 그 버튼이 들어 있습니다.
...
>
> 이 OIDC 설정은 IAM 신뢰 정책을 손으로 써야 해서 오늘 수업 범위를 넘어갑니다. **제가 이미 다 해뒀습니다.** 여러분 저장소의 Settings → Secrets에 `AWS_DEPLOY_ROLE_ARN` 이라는 이름으로 들어가 있습니다. 여러분은 템플릿에서 그 이름을 부르기만 하면 됩니다. 부록 A에 제가 만든 신뢰 정책을 참고용으로 넣어뒀으니, 궁금한 사람은 나중에 읽어보세요.
>
...
- 권한은 **OIDC 임시 자격증명** (영구 키 저장 안 함) — 강사 사전 구성
...
- 12주차부터 여러분은 코드를 고칠 때마다 SSH로 들어가지 않습니다. **13주차 통합 작업 속도가 여기서 갈립니다.** 파이프라인이 없는 팀은 13주차에 통합할 때마다 7단계를 손으로 밟아야 합니다.
