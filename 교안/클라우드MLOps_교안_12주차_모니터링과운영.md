...
| 항목 | 내용 |
|---|---|
| 학습목표 | 서비스의 로그·메트릭을 CloudWatch로 수집해 대시보드로 보고, 에러율이 기준을 넘으면 이메일 알람을 받을 수 있다. 입력 데이터가 학습 때와 달라지는지 감시하는 간이 스크립트를 만들 수 있다. |
| 오늘의 결과물 | ① CloudWatch 대시보드(추론 횟수·응답시간·에러율·서버 자원) ② 실제로 수신된 알람 이메일 ③ 드리프트 점검 스크립트 실행 결과 ④ 팀 운영 체크리스트 1장 ⑤ **M3 전 구간 연결 확인표** |
| 사전 준비 | ① EC2 인스턴스 역할에 `CloudWatchAgentServerPolicy` 부착 ② `amazon-cloudwatch-agent` 설치 스크립트 사전 배포 ③ 학습 데이터 기준통계 파일 `baseline_stats.json` 을 공통 예제용으로 S3에 사전 업로드 ④ SNS 주제 생성 실습이 막힐 경우를 대비한 강사 공용 주제 1개 ⑤ 완성본 태그 `week-12-done` |
| 학생 준비물 | 노트북, 11주차에 완성한 자동 배포 저장소, 동작 중인 EC2 API, **수신 확인 메일을 열 수 있는 이메일 계정**(학교 메일 스팸함 주의) |
| 예상 사고 지점 | ① CloudWatch Agent 설정 JSON 오타로 에이전트가 조용히 죽음 ② EC2 인스턴스 역할에 CloudWatch 권한이 없어 메트릭이 안 올라옴 ③ SNS 구독 확인 메일을 안 눌러 알람이 영영 안 옴 |
...
- 확인 포인트: 11주차 파이프라인이 자동으로 돌아 재배포된 뒤, EC2에서 `ls -l /var/log/mlops/app.log` 로 파일이 생성됐는지 본다.
...
> 지금부터 세 가지를 합니다. 첫째, 여러분 학습 데이터의 평균과 표준편차를 파일 하나로 뽑아서 S3에 올립니다. 이게 "정상이란 이런 모습이다"라는 **기준선**입니다. 둘째, 최근 API로 들어온 입력들을 그 기준선과 비교해서 얼마나 벗어났는지 점수를 냅니다. 셋째, 여러분 팀이 매일 아침 3분 동안 볼 체크리스트를 다섯 줄로 만듭니다. 그리고 마지막 30분에는 **M3 점검표**를 팀별로 통과해야 합니다. M3는 오늘 반드시 넘어갑니다. 여기서 안 넘어가면 13주차 통합 작업이 무너집니다.
...
- `docs/운영체크리스트.md` 는 14주차 발표의 "운영" 슬라이드 근거 자료가 됩니다. 13주차 코칭에서 이 파일 유무를 확인합니다.
...
> 미통과 항목이 있는 팀은 **13주차 코칭 시작 시점에 그 항목부터** 잡습니다.
...
1. **(팀·필수)** 오늘부터 14주차까지 **일일 점검 기록을 최소 5회분** 채운다. `docs/운영체크리스트.md` 3장 표에 날짜·담당·결과를 적고 커밋한다. **커밋 날짜가 실제로 분산되어 있어야 인정한다.**
