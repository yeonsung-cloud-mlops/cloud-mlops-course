...
| 사전 준비 | ① 팀별 코칭 순서표와 시각 배정(칠판 게시) ② 코칭 체크리스트 인쇄본 팀 수만큼 ③ 화면 녹화 도구 안내(Windows `Win+G`, macOS `Cmd+Shift+5`, OBS) ④ 발표자료 템플릿(부록 C) 배포 ⑤ 14주차 발표 순서 추첨용 제비 ⑥ 각 팀의 M3 미통과 항목 정리표 |
| 학생 준비물 | 노트북, 팀 저장소, 동작 중인 서비스, 12주차 운영체크리스트, **발표에서 보여줄 실제 입력 데이터 3건**, 녹화용 여유 저장공간 2GB |
...
| 03:35–03:50 | 마무리 — 제출 확인, 발표 순서 추첨, 14주차 안내 | 15 |
| **합계** | | **230** |
...
# ③ SageMaker 엔드포인트 — 9주차 이후 남아 있으면 큰일 ⚠
...
- **`2001070405_20v1.2` 인공지능서비스 운영품질 점검하기**
  - 정의한 **품질지표에 따라 서비스 상태를 실제로 점검**했는가 → 12주차에 정의한 지표(응답시간·에러율·드리프트)를 오늘 실측하고 기준 대비 판정했는가
...
| 10 | 운영·모니터링 | 대시보드 캡처 + 알람 메일 + 재학습 정책 | 12:40–13:30 |
