...
| 사전 준비 | ① 발표 순서표(13주차 추첨분) 게시 ② 프로젝터·HDMI·USB-C 젠더·백업 노트북 ③ 타이머(15분/5분 벨) ④ 상호평가지 인쇄(학생 수 × 팀 수) ⑤ Cost Explorer 팀 태그별 화면 미리 준비 ⑥ 정리 확인 스크립트 `cleanup_check.sh` 배포 ⑦ **강사 계정 기준 최종 스윕 목록** 준비 |
...
> **8팀 이상이면 발표를 12분으로 단축**하고, 질의는 3분으로 줄입니다. 이 경우 발표자료 구성표(13주차 부록 C)에서 4장(데이터)과 12장(한계)을 각 30초씩 압축하도록 사전 공지합니다.
...
> 14주 전에 여러분 대부분은 클라우드 콘솔에 로그인해본 적도 없었습니다. 지금은 모델을 학습시켜서, 컨테이너에 담아서, 서버에 올려서, 자동으로 배포되게 하고, 죽으면 메일이 오게 만들 수 있습니다. **이 전 구간을 혼자 관통해본 사람은 생각보다 많지 않습니다.** 대학 4년 내내 주피터 노트북 안에서만 있다가 졸업하는 사람이 훨씬 많아요.
...
>
> 그 아래는 13주차 부록 C에서 쓴 그대로 두면 됩니다. 다만 **오늘 이후로 반드시 추가할 게 두 가지** 있습니다.
>
...
>
> 마지막으로 하나. **저장소를 공개(Public)로 전환**하세요. 비공개면 아무도 못 봅니다. 전환 전에 `.env`, 키 파일, 개인정보가 포함된 데이터가 없는지 **반드시** 확인하세요. 11주차에 말씀드린 대로입니다.
...
>
> 가장 가성비가 좋은 건 **AWS Certified Cloud Practitioner** 입니다. 여러분은 이미 콘솔·IAM·S3·EC2·비용 개념을 다 만져봤습니다. 4~6주 준비하면 붙습니다. 그 다음이 **AWS Certified Solutions Architect – Associate**. 난이도가 한 단계 올라가지만 취업 시장에서 인지도가 가장 높습니다. **AWS Certified Machine Learning – Specialty**는 SageMaker 비중이 커서 9주차 경험이 도움이 되지만, 실무 경험 없이 바로 도전하기엔 무겁습니다.
>
...
> **2순위는 워크플로 오케스트레이션.** Airflow나 Prefect입니다. 우리는 GitHub Actions로 배포만 자동화했는데, 실무에서는 "매일 새벽 3시에 데이터 받아서 전처리하고 학습하고 평가해서 좋으면 배포" 같은 흐름을 짜야 합니다. 11주차에 배운 "오케스트레이션"이 이겁니다.
...
| | 4 | AWS Certified Machine Learning – Specialty (실무 경험 후) |
...
> "여러분은 이제 '주소를 가진 모델'을 만들어본 사람입니다. 14주 동안 수고 많았습니다."
...
- **`2001070405_20v1.3` 인공지능서비스 운영품질 개선하기(부분)**
  - 운영 중 확인된 **품질 문제를 개선하고 결과를 확인**했는가 → 13주차 교차 테스트 지적사항 반영 및 재현 성공
...
> 3~10장의 상세 양식은 13주차 부록 C(README 필수 항목)를 그대로 사용합니다.
...
| SLI | Service Level Indicator | 서비스 수준을 재는 실제 숫자. 12주차 에러율이 여기 해당 |
